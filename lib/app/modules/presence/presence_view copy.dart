          ...
          if (homeC.isLemburPresence.value) {
            ...
            if (!((presenceData.value ?? {}).containsKey("lemburan"))) {
              if (presenceData.value?["shift"] == "0") {
                lemburTime.putIfAbsent(
                    "manual", () => {"lembur_masuk": now.toString()});
                ...
                ((presenceData.value ?? {})["lemburan"] as Map)
                    .containsKey("manual") &&
                !((presenceData.value?["lemburan"]["manual"] as Map)
                    .containsKey("lembur_keluar"))) {
              if (presenceData.value?["shift"] == "0") {
                (lemburTime["manual"] as Map)
                    .putIfAbsent("lembur_keluar", () => now.toString());
                if (lemburTime.isNotEmpty) {
                  print("lembur : $lemburTime");
                  body?.putIfAbsent("lemburan", () => lemburTime);
                }
            ...
            if (resLogout) {
              if ((presenceData.value ?? {}).containsKey("status")) {
                body?.putIfAbsent(
                    "status", () => presenceData.value?["status"]);
              }
            }
          } else if ((presenceData.value ?? {}).containsKey("login_presence") &&
              (presenceData.value ?? {}).containsKey("logout_presence")) {
            body?.putIfAbsent(
                "login_presence", () => presenceData.value?['login_presence']);
            body?.putIfAbsent(
                "logout_presence", () => presenceData.value?['login_presence']);
          } else if (!((presenceData.value ?? {})
              .containsKey("login_presence"))) {
            ...
            if (body?["shift"] == "0") {
              ...
              if (now.isAfter(Utils.customDate(
                  split(shiftPagi["jamKeluar"], true),
                  split(shiftPagi["jamKeluar"], false)))) {
                lemburTime.putIfAbsent("auto_keluar", () {
                  return {
                    "lembur_keluar": now.toString(),
                    "lembur_masuk": Utils.customDate(
                            split(shiftPagi["jamKeluar"], true),
                            split(shiftPagi["jamKeluar"], false))
                        .toString(),
                  };
              ...
              if (DateTime.parse(presenceData.value?['login_presence'])
                  .isBefore(Utils.customDate(split(shiftPagi["jamMasuk"], true),
                      split(shiftPagi["jamMasuk"], false)))) {
                lemburTime.putIfAbsent("auto_masuk", () {
                  return {
                    "lembur_masuk":
                        presenceData.value?['login_presence'].toString(),
              ...
              // lembur by sistem ketika presence keluar
              if (now.isAfter(Utils.customDate(
                  split(shiftSore["jamKeluar"], true),
                  split(shiftSore["jamKeluar"], false)))) {
                lemburTime.putIfAbsent("auto_keluar", () {
                  return {
                    "lembur_keluar": now.toString(),
                    "lembur_masuk": Utils.customDate(
              ...
              // lembur by sistem ketika presence masuk
              if (DateTime.parse(presenceData.value?['login_presence'])
                  .isBefore(Utils.customDate(split(shiftSore["jamMasuk"], true),
